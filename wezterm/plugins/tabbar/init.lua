...
wez.on("update-status", function(window, pane)
	...
	if options.modules.pane.enabled then
		...
		local tt = utilities._space(utilities._basename(process), options.separator.space)

		table.insert(left_cells, {
			Text = options.modules.pane.icon .. tt:gsub(utilities.home, "~"),
		})
	end
	...
	local callbacks = {
		{
			name = "spotify",
			func = function()
				return spotify.get_currently_playing(
					options.modules.spotify.max_width,
					options.modules.spotify.throttle
				)
			end,
		},
		{
